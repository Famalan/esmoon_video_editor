...
4. Находит самостоятельные фрагменты через Codex CLI: вход по подписке ChatGPT, модель `gpt-5.6-luna`, `reasoning effort=max`. Длительность не задаётся: один ролик должен раскрывать одну боль и доводить её до законченного решения.
